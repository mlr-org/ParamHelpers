...
test_that("getTypes", {
  ...
  expect_equal(
    getTypes(ps, df.cols=FALSE, use.names=FALSE),
    c("discrete", "numericvector", "integer")
  )
  expect_equal(
    getTypes(ps, df.cols=TRUE, use.names=FALSE),
    c("factor", "numeric", "numeric", "integer")
  )
  expect_equal(
    getTypes(ps, df.cols=FALSE, use.names=TRUE),
    c(x="discrete", y="numericvector", z="integer")
  )
  expect_equal(
    getTypes(ps, df.cols=TRUE, use.names=TRUE, with.nr=FALSE),
    c(x="factor", y="numeric", y="numeric", z="integer")
  )
  expect_equal(
    getTypes(ps, df.cols=TRUE, use.names=TRUE, with.nr=TRUE),
