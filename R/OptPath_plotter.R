...
#' @param marked [\code{integer()}]\cr
#'   Indices of points that should be marked in the plots. Default is \code{NULL} 
#'   (no points are marked).
#' @return List of plots, one for each iteration.
#' @export
#' 
plotOptPath = function(op, iters, pause = TRUE, alpha = TRUE, lim.x = list(), 
  lim.y = list(), title = "", colours = c("red", "blue", "green", "orange"), size = c(3, 1.5), 
  impute.scale = 1, impute.value = "missing", scale = "std", 
  ggplot.theme = ggplot2::theme(legend.position = "top"), marked = NULL) {
  ...
  if (!is.null(marked)) asInteger(marked)
