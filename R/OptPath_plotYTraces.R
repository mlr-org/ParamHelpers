...
renderYTraces = function(opt.paths) {
  ...
  fronts = lapply(seq_along(opt.paths), function(j) {
    ...
    df = data.frame(
      ...
      dob = getOptPathDOB(op = run)
    )
    cns = colnames(df)
    if (length(cns) != 2L) {
      stopf("Must always have 1 objective in opt path. But found: %i", length(cns) - 1)
    }
    if (j == 1L) {
      y.name <<- cns[1]
    ...
    if (!y.name == cns[1]) {
      stopf("Must always have the same objective in opt path: %s (first ones taken). But found here: %s",
        collapse(y.name), collapse(cns[1]))
    }
  ...
  mean.data = aggregate(data[[y.name]], by = list(data$dob, data$.algo), FUN = mean)
  names(mean.data) = c("dob", ".algo", y.name)
  
  pl = ggplot2::ggplot(data, ggplot2::aes_string(x = "dob", y = y.name, group = ".algo", 
...
plotYTraces = function(opt.paths) {
  print(renderYTraces(opt.paths))
