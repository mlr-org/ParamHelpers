...
test_that("ParamSetSugar generates the expected ParamSets", {
  ...
  param.sets = list(
      list(makeParamSet(makeIntegerLearnerParam("a"), makeIntegerLearnerParam("b")),
        pSSLrn(a: integer[, ], b = NA: integer[, ])),
...
test_that("ParamSetSugar works with params that have no bounds", {
  ...
  ps.sugar = pSS(a = TRUE: logical, b = identity: funct, c = "test": character, d = list(1, 2, 3): untyped)
