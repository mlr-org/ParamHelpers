...
test_that("no changes", {
  ...
  expect_identical(evaluateParamSet(ps, envir = list(p = 3)), ps)
  ...
  expect_identical(evaluateParamSet(ps, envir = list(p = 3, z = "b")), ps)
  expect_identical(evaluateParamSet(ps), ps)
  expect_identical(evaluateParamSet(ps, envir = list(p = 3)), ps)
  expect_identical(evaluateParamSet(ps, envir = list(z = "b")), ps)
})
...
test_that("expressions", {
  ...
  ee = list(p = 3, z = "b")
  expect_identical(evaluateParamSet(ps, envir = ee), ps2)
  expect_error(evaluateParamSet(ps))
  expect_error(evaluateParamSet(ps, envir = list(p = 3)))
  expect_error(evaluateParamSet(ps, envir = list(z = "b")))
})
