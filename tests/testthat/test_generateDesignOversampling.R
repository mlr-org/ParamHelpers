...
test_that("generateDesignOversampling", {
  ...
  expect_true(is.na(des[, 2L]) || des[, 2L] >= 15 && des[, 2L] <= 45)
  expect_true(des[, 1L] == "a" || is.na(des[, 2L]))
  expect_true(is.na(des[, 3L]) || des[, 3L] >= 1 && des[, 3L] <= 120)
  expect_true(des[, 1L] == "a" || is.na(des[, 3L]))
  expect_true(is.na(des[, 4L]) || des[, 4L] >= -15 && des[, 4L] <= 15)
  expect_true(des[, 1L] == "b" || is.na(des[, 4L]))
  expect_true(is.na(des[, 5L]) || des[, 5L] %in% c("linear", "radial"))
  expect_true(des[, 1L] == "b" || is.na(des[, 5L]))
  expect_true(is.na(des[, 6L]) || des[, 6L] >= -15 && des[, 6L] <= 15)
  expect_true(des[, 1L] == "b" && des[, 5L] == "radial" || is.na(des[, 5L]))
  expect_true(is.na(des[, 7L]) || des[, 7L] >= 1 && des[, 7L] <= 21)
  expect_true(des[, 1L] == "c" || is.na(des[, 7L]))
})
